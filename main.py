...
configFilePath = f"{os.environ['LOCALAPPDATA']}\\temp\\Dusk"
...
class configListener(dict):
    ...
    def __setitem__(self, item, value):
        ...
        if hasattr(DuskClass, "config"):
            json.dump(DuskClass.config, open(configFilePath, "w", encoding="utf-8"), indent=4)
    ...
    def esp(self):
        ...
        while pm.overlay_loop():
            ...
            if self.config["misc"]["watermark"]:
                watermark = f"Dusk | {pm.get_fps()} fps"
...
if __name__ == "__main__":
    if os.name != "nt":
        input("Dusk is only working on Windows.")
    ...
    def toggleEsp(id, value):
        DuskClass.config["esp"]["enabled"] = value

        if value and not DuskClass.overlayThreadExists:
            threading.Thread(target=DuskClass.esp, daemon=True).start()
    ...
    def setBindEsp(id, value):
        ...
        if waitingForKeyEsp:
            DuskClass.config["esp"]["bind"] = value
    ...
    def toggleEspBox(id, value):
        DuskClass.config["esp"]["box"] = value

    def toggleEspBoxBackground(id, value):
        DuskClass.config["esp"]["boxBackground"] = value

    def toggleEspSkeleton(id, value):
        DuskClass.config["esp"]["skeleton"] = value

    def toggleEspRedHead(id, value):
        DuskClass.config["esp"]["redHead"] = value

    def toggleEspSnapline(id, value):
        DuskClass.config["esp"]["snapline"] = value

    def toggleEspOnlyEnemies(id, value):
        DuskClass.config["esp"]["onlyEnemies"] = value

    def toggleEspName(id, value):
        DuskClass.config["esp"]["name"] = value

    def toggleEspHealth(id, value):
        DuskClass.config["esp"]["health"] = value

    def setEspColor(id, value):
        DuskClass.config["esp"]["color"] = {"r": value[0], "g": value[1], "b": value[2], "a": value[3]}

        DuskClass.espColor = pm.new_color_float(value[0], value[1], value[2], value[3])
        DuskClass.espBackGroundColor = pm.fade_color(DuskClass.espColor, 0.3)

    def setEspBoxRounding(id, value):
        DuskClass.config["esp"]["boxRounding"] = value

    def toggleTriggerBot(id, value):
        DuskClass.config["triggerBot"]["enabled"] = value

        if value:
            threading.Thread(target=DuskClass.triggerBot, daemon=True).start()
    ...
    def setBindTriggerBot(id, value):
        ...
        if waitingForKeyTriggerBot:
            DuskClass.config["triggerBot"]["bind"] = value
    ...
    def toggleTriggerBotOnlyEnemies(id, value):
        DuskClass.config["triggerBot"]["onlyEnemies"] = value

    def sliderTriggerBotDelay(id, value):
        DuskClass.config["triggerBot"]["delay"] = value

    def toggleBunnyHop(id, value):
        DuskClass.config["misc"]["bhop"] = value

        if value:
            threading.Thread(target=DuskClass.bhop, daemon=True).start()
            
    def toggleNoFlash(id, value):
        DuskClass.config["misc"]["noFlash"] = value

        DuskClass.noFlash()

    def toggleWatermark(id, value):
        DuskClass.config["misc"]["watermark"] = value

        if value and not DuskClass.overlayThreadExists:
            threading.Thread(target=DuskClass.esp, daemon=True).start()

    def toggleSaveSettings(id, value):
        DuskClass.config["settings"]["saveSettings"] = value

    def toggleStreamProof(id, value):
        DuskClass.config["settings"]["streamProof"] = value

        if value:
            user32.SetWindowDisplayAffinity(DuskClass.guiWindowHandle, 0x00000011)
            user32.SetWindowDisplayAffinity(DuskClass.overlayWindowHandle, 0x00000011)
        else:
            user32.SetWindowDisplayAffinity(DuskClass.guiWindowHandle, 0x00000000)
            user32.SetWindowDisplayAffinity(DuskClass.overlayWindowHandle, 0x00000000)

    def toggleAlwaysOnTop(id, value):
        if value:
            win32gui.SetWindowPos(DuskClass.guiWindowHandle, win32con.HWND_TOPMOST, 0, 0, 0, 0, win32con.SWP_NOMOVE | win32con.SWP_NOSIZE)
        else:
            win32gui.SetWindowPos(DuskClass.guiWindowHandle, win32con.HWND_NOTOPMOST, 0, 0, 0, 0, win32con.SWP_NOMOVE | win32con.SWP_NOSIZE)
        ...
        with dpg.tab_bar():
            with dpg.tab(label="ESP"):
                ...
                with dpg.group(horizontal=True):
                    checkboxToggleEsp = dpg.add_checkbox(label="Toggle", default_value=DuskClass.config["esp"]["enabled"], callback=toggleEsp)
                    buttonBindEsp = dpg.add_button(label="Click to Bind", callback=statusBindEsp)

                    bind = DuskClass.config["esp"]["bind"]
                    if bind != 0:
                ...
                with dpg.group(horizontal=True):
                    checkboxEspBox= dpg.add_checkbox(label="Box", default_value=DuskClass.config["esp"]["box"], callback=toggleEspBox)
                    checkboxEspBackground = dpg.add_checkbox(label="Background", default_value=DuskClass.config["esp"]["boxBackground"], callback=toggleEspBoxBackground)

                with dpg.group(horizontal=True):
                    checkboxEspSkeleton= dpg.add_checkbox(label="Skeleton", default_value=DuskClass.config["esp"]["skeleton"], callback=toggleEspSkeleton)
                    checkboxEspSkeleton= dpg.add_checkbox(label="Red Head", default_value=DuskClass.config["esp"]["redHead"], callback=toggleEspRedHead)

                checkboxEspSnapline= dpg.add_checkbox(label="Snapline", default_value=DuskClass.config["esp"]["snapline"], callback=toggleEspSnapline)
                checkboxEspOnlyEnemies = dpg.add_checkbox(label="Only Enemies", default_value=DuskClass.config["esp"]["onlyEnemies"], callback=toggleEspOnlyEnemies)
                checkboxEspName = dpg.add_checkbox(label="Show Name", default_value=DuskClass.config["esp"]["name"], callback=toggleEspName)
                checkboxEspHealth = dpg.add_checkbox(label="Show Health", default_value=DuskClass.config["esp"]["health"], callback=toggleEspHealth)

                dpg.add_spacer(width=75)
                dpg.add_separator()
                dpg.add_spacer(width=75)

                colorPickerEsp = dpg.add_color_picker(label="Global Color", default_value=(DuskClass.config["esp"]["color"]["r"]*255, DuskClass.config["esp"]["color"]["g"]*255, DuskClass.config["esp"]["color"]["b"]*255, DuskClass.config["esp"]["color"]["a"]*255), width=150, no_inputs=True, callback=setEspColor)
                sliderEspBoxRounding = dpg.add_slider_float(label="Box Rounding", default_value=DuskClass.config["esp"]["boxRounding"], min_value=0, max_value=1, clamped=True, format="%.1f", callback=setEspBoxRounding, width=250)
            with dpg.tab(label="TriggerBot"):
                dpg.add_spacer(width=75)

                with dpg.group(horizontal=True):
                    checkboxToggleTriggerBot = dpg.add_checkbox(label="Toggle", default_value=DuskClass.config["triggerBot"]["enabled"], callback=toggleTriggerBot)
                    buttonBindTriggerBot = dpg.add_button(label="Click to Bind", callback=statusBindTriggerBot)
                    bind = DuskClass.config["triggerBot"]["bind"]
                    if bind != 0:
                ...
                checkboxTriggerBotOnlyEnemies = dpg.add_checkbox(label="Only Enemies", default_value=DuskClass.config["triggerBot"]["onlyEnemies"], callback=toggleTriggerBotOnlyEnemies)
                
                dpg.add_spacer(width=75)
                dpg.add_separator()
                dpg.add_spacer(width=75)

                sliderDelayTriggerBot = dpg.add_slider_float(label="Shot Delay", default_value=DuskClass.config["triggerBot"]["delay"], max_value=1, callback=sliderTriggerBotDelay, width=250, clamped=True, format="%.1f")
            with dpg.tab(label="Misc"):
                dpg.add_spacer(width=75)

                with dpg.group(horizontal=True):
                    checkboxBhop = dpg.add_checkbox(label="BunnyHop", default_value=DuskClass.config["misc"]["bhop"], callback=toggleBunnyHop)
                    dpg.add_text(default_value="| Hold space!")
                    
                checkboxNoFlash = dpg.add_checkbox(label="NoFlash", default_value=DuskClass.config["misc"]["noFlash"], callback=toggleNoFlash)
                checkboxWatermark = dpg.add_checkbox(label="Watermark", default_value=DuskClass.config["misc"]["watermark"], callback=toggleWatermark)
            with dpg.tab(label="Settings"):
                dpg.add_spacer(width=75)

                checkboxSaveSettings = dpg.add_checkbox(label="Save Settings", default_value=DuskClass.config["settings"]["saveSettings"], callback=toggleSaveSettings)

                dpg.add_spacer(width=75)

                checkboxStreamProof = dpg.add_checkbox(label="Stream Proof", default_value=DuskClass.config["settings"]["streamProof"], callback=toggleStreamProof)
    ...
    DuskClass.guiWindowHandle = win32gui.FindWindow(title, None)
    if DuskClass.config["settings"]["streamProof"]:
        user32.SetWindowDisplayAffinity(DuskClass.guiWindowHandle, 0x00000011)
